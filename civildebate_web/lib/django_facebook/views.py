...
from greatdebate.settings import INSTALLATION_ROOT

import logging

LOG_FN = '%slog/django_facebook.views.log' % (INSTALLATION_ROOT)

logger = logging.getLogger("dj-fb.views")
        ...
        if facebook.is_authenticated():
            ...
            if request.user.is_authenticated():
                ...
                if authenticated_user:
                    return _login_user(request, facebook, authenticated_user, True)
                else:
...
def _register_user(request, facebook, authenticated_user, profile_callback=None):
    ...
    if facebook.is_authenticated():
        ...
        logger.debug("fbdata, profile data _register")
    ...
    if request.method == 'POST':
        ...
        logger.debug("...form _register")
        logger.debug(form)

        if form.is_valid():
            new_user = form.save(profile_callback=profile_callback, send_email=False)
            ...
            dbf = DebateFacebookProfile.objects.get(user=new_user)
            dbf.facebook_id = profile['id']
            dbf.save()
            
            logger.debug("...new user _register")
            ...
            if email and email_verified:
                kwargs = {'facebook_email': email}
            authenticated_user = authenticate(facebook_id=profile['id'], **kwargs)
            logger.debug("...auth user _register")
            logger.debug(authenticated_user)
            if authenticated_user:
                return _login_user(request, facebook, authenticated_user, True)
    else:
