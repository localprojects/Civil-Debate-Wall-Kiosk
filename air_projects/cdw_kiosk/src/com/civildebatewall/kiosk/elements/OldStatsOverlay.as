...
//			wordSearchResults.scrollField.scrollSheet.addEventListener(Comment.EVENT_DEBATE, onDown, true);			
		...
		private function onDataChange(e:Event):void {
			...
			mostDebatedView(); // TODO remove this!
			showDebateTotals();			
		}
		...
		public function frequentWordView(...args):void {
			...
			homeButton.setDownColor(CivilDebateWall.state.activeThread.firstPost.stanceColorMedium);			
		...
		public function mostDebatedView(...args):void {
			...
			homeButton.setBackgroundColor(CivilDebateWall.state.activeThread.firstPost.stanceColorLight, true);
			homeButton.setDownColor(CivilDebateWall.state.activeThread.firstPost.stanceColorMedium);			
			wordCloud.deselect();
			...
			superlativesPortrait.setPost(CivilDebateWall.data.mostDebatedThreads[0].firstPost, true);
		...
		public function mostLikedView(...args):void {
			...
			superlativesPortrait.setPost(CivilDebateWall.data.mostLikedPosts[0], true);			
...
//		}				
		...
		public function showDebateTotals(...args):void {			
			...
			var yesCount:uint = CivilDebateWall.data.stanceTotals['yes'];
			var noCount:uint = CivilDebateWall.data.stanceTotals['no'];			
		...
		public function update():void {
			...
			wordCloud.setWords(CivilDebateWall.data.frequentWords);
			
			// pull out the first posts so we're working with posts instead of threads
			var mostDebatedFirstPosts:Array = [];
			for (var i:uint = 0; i < CivilDebateWall.data.mostDebatedThreads.length; i++) {
			...
			trace("most liked: " + CivilDebateWall.data.mostLikedPosts);
			mostLikedList.setItems(CivilDebateWall.data.mostLikedPosts);
		}		
