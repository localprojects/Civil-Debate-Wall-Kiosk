package com.civildebatewall.wallsaver.sequences {
	...
	public class BarGraphSequence extends Sprite implements ISequence {
		...
		public function getTimelineIn():TimelineMax	{
			...
			var yesBarTweenIn:TweenMax = TweenMax.fromTo(yesBar, yesBarScrollDuration, {x: -yesBar.width + Main.physicalScreenWidth}, {x: -arrowWidth + Main.physicalScreenWidth, ease: Quart.easeOut});
			
			var noBarScrollDuration:int = (Main.totalWidth - Main.physicalScreenWidth - noWidth - arrowWidth)  / scrollVelocity;						
			var noBarTweenIn:TweenMax = TweenMax.fromTo(noBar, noBarScrollDuration, {x: Main.totalWidth}, {x: Main.totalWidth - noWidth - arrowWidth, ease: Quart.easeOut});			
		...
		public function getTimelineOut():TimelineMax {
			...
			var noBarOutDuration:int = (Main.totalWidth - noWidth - arrowWidth)  / (scrollVelocity - 10);
