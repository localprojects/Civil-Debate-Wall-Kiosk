...
package uk.co.soulwire.gui
{
	...
	public class SimpleGUI extends EventDispatcher
	{
		...
		public function addTextArea(target:String, options:Object = null):void {
			options = parseOptions(target, options);

			var params:Object = {onChange: onTextAreaChange};			
			
			params.target = target;			
			
			addControl(TextArea, merge(params, options));			
		}
			...
			if (options.hasOwnProperty("target"))
			{
				...
				for (i = 0; i < targets.length; i++)
				{
					...
					else if (component is ComboBox)
					{
						...
						target[prop] = component["text"]; 
					}
					else if(component.hasOwnProperty("value"))
					{
						//trace("setting text: " + prop + " to " + component["value"]);
